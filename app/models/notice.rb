class Notice < ActiveRecord::Base
  ...
  def self.commenters(comment)
    ...
    Notice.new( # subtract comment creator and record creator
  ...
  def self.like(like, liked)
    if like.user != like.post.user
      like_notice = Notice.new(user_ids: [like.post.user_id],
  ...
  def self.follow(follower, following, follow)
    ...
    follow_notice = Notice.new(user_ids: [following.id],
                               user_id: follower.id,
                               link: "follow",
                               notice_type: "tip")
    if follow
